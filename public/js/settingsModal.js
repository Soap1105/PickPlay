...
(function () {
    ...
    function collectSettings(game) {
        if (game === 'bingo') {
            ...
            // hidden 필드에도 반영 (bingo.js가 읽을 수 있도록)
            ...
            return { winLines, turnOrder, turnTime, topic };
            ...
            const winTarget = parseInt(modalBody.querySelector('#modal-bomb-win-target').value);
    ...
    function renderModalContent(game) {
        ...
        if (game === 'bingo') {
            ...
            modalTitle.textContent = '빙고 게임 설정';
            modalSubtitle.textContent = '게임 규칙과 AI 주제를 설정하세요';
            modalBody.innerHTML = `
                ...
                <div class="smodal-section">
                    ...
                    <select id="modal-turn-order" class="smodal-select">
                        ...
                        <option value="random" ${hTurnOrder==='random'?'selected':''}>🎲 완전 랜덤</option>
                        <option value="join_asc" ${hTurnOrder==='join_asc'?'selected':''}>➡️ 입장 순서대로</option>
                        <option value="join_desc" ${hTurnOrder==='join_desc'?'selected':''}>⬅️ 입장 역순으로</option>
                    </select>
                </div>
                <div class="smodal-section">
                    <label class="smodal-label">⏱️ 턴 제한 시간</label>
                    <select id="modal-turn-time" class="smodal-select">
                        ...
                        <option value="15" ${hTurnTime==='15'?'selected':''}>15초 (기본)</option>
                ...
                <div class="smodal-section">
                    ...
                    <p style="font-size:0.8rem; color:#aaa; margin: 6px 0 0;">입력 시 AI가 해당 주제의 단어 25개를 자동 생성합니다.</p>
                </div>
            `;
            ...
            modalSubtitle.textContent = '승리 조건과 카테고리를 선택하세요';
            modalBody.innerHTML = `
                <div class="smodal-section">
                ...
                <div class="smodal-section">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                        <label class="smodal-label" style="margin:0;">📋 카테고리 선택</label>
            ...
            const hWinTarget = document.getElementById('bomb-win-target')?.value || '3';
            ...
            modalSubtitle.textContent = '게임 옵션과 출제 카테고리를 선택하세요';
            modalBody.innerHTML = `
                <div class="smodal-section">
                    <label class="smodal-label">🏆 목표 승수</label>
                    <select id="modal-bomb-win-target" class="smodal-select">
                        <option value="2" ${hWinTarget==='2'?'selected':''}>2승</option>
                        <option value="3" ${hWinTarget==='3'?'selected':''}>3승 (기본)</option>
                        <option value="4" ${hWinTarget==='4'?'selected':''}>4승</option>
                        <option value="5" ${hWinTarget==='5'?'selected':''}>5승</option>
