...
1. 제1원칙 (정확성): 무조건 해당 주제 "${topic}"에 완벽히 부합하는 실존하는 고유 명칭이어야 해. 40개를 채우기 위해 근거 없는 단어를 지어내는 것은 절대 금지야.
2. 제2원칙 (수량): 1원칙을 지키면서 최소 40개를 찾아내. 만약 실존하는 단어가 40개 미만이라면 억지로 채우지 말고 무조건 실패(error)를 반환해.
3. 제3원칙 (대중성): 1, 2원칙이 충족되는 선에서, 가급적 한국인에게 친숙하고 대중적인 단어를 우선적으로 리스트 상단에 배치해. 희귀하고 전문적인 단어는 40개를 채우기 위해 부족한 경우에만 보조적으로 포함해.

### [카테고리별 특화 규칙]
1. 인물/캐릭터: 일본식(성 이름), 서양식/한국식 표준 풀네임을 사용해. 관계 묘사(X의 언니 등)는 금지하고 진짜 이름만 사용해.
2. 미디어(노래, 영화 등): 오직 순수 제목만 추출해. 아티스트명, 제작사, 연도 등 부가 정보는 제외해.
...
2. 길이 제한: 각 단어는 최대 12글자 이내여야 해. 12글자를 초과하는데 널리 쓰이는 공식 줄임말도 없다면, **글자를 억지로 자르지 말고 아예 그 단어는 목록에서 빼고 다른 단어로 대체해.** (의미 훼손 방어 및 UI 보호)
3. 검색 출처(각주) 강제 제거: 단어 뒤에 [1], [2] 같은 검색 참조 번호가 붙어있다면 무조건 삭제하고 순수한 명칭만 남겨.

### [Self-Correction 최종 검토]
결과를 출력하기 직전에 모든 단어를 훑어보고, "이 단어가 주제에 맞는 진짜 이름이 맞는가?", "중복이나 각주 표기([1])는 없는가?", "12글자를 초과하는 단어는 없는가?"를 자문해라. 40개를 채우려고 '이상한 단어'를 넣었다면 즉시 삭제하고 정답으로 교체하거나, 정답이 없으면 실패(error)를 반환해.

[출력 형식 - 다른 인사말이나 부연 설명 없이 오직 JSON만 반환할 것]
